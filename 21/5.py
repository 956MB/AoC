...
def _main():
    use_x = 0
    use_y = 0
    inp.sort(key=lambda x: int(x[0][0]))
    max_x = inp[-1][0][0]
    if max_x >= use_x: use_x = max_x
    inp.sort(key=lambda x: int(x[0][1]))
    max_y = inp[-1][0][1]
    if max_y >= use_y: use_y = max_y
    inp.sort(key=lambda x: int(x[1][0]))
    max_x = inp[-1][1][0]
    if max_x >= use_x: use_x = max_x
    inp.sort(key=lambda x: int(x[1][1]))
    max_y = inp[-1][1][1]
    if max_y >= use_y: use_y = max_y

    matrix = [[0 for _ in range(use_x+1)] for _ in range(use_y+1)]
    ...
    for pair in inp:
        ...
        if start_x == end_x:
            st_y = start_y if start_y<=end_y else end_y
            en_y = start_y if start_y>=end_y else end_y
            for _y in range(st_y, en_y+1):
                matrix[_y][start_x] = matrix[_y][start_x] + 1
        elif start_y == end_y:
            st_x = start_x if start_x<=end_x else end_x
            en_x = start_x if start_x>=end_x else end_x
            for _x in range(st_x, en_x+1):
                matrix[start_y][_x] = matrix[start_y][_x] + 1
        else:
            st_x = start_x
            st_y = start_y
            en_x = end_x
            en_y = end_y
            matrix[st_y][st_x] = matrix[st_y][st_x] + 1
            while st_x != en_x and st_y != en_y:
