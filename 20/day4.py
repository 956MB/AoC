...
def _pt2():
    ...
    for v1 in _input:
        ...
        a = [v.split(':')[0] for v in l]
        if "cid" in a:
            if set(a) != set(checks2): continue
        else:
            if set(a) != set(checks1): continue
        ...
        for v2 in l:
            ...
            if pre == 'byr':
                if not _check_byr(pos, v2): br = True; break
            if pre == 'iyr':
                if not _check_iyr(pos, v2): br = True; break
            if pre == 'eyr':
                if not _check_eyr(pos, v2): br = True; break
            if pre == 'hgt':
                if not _check_hgt(pos, v2): br = True; break
            if pre == 'hcl':
                if not _check_hcl(pos, l): br = True; break
            if pre == 'ecl':
                if not _check_ecl(pos, v2): br = True; break
            if pre == 'pid':
                if not _check_pid(pos, l): br = True; break
...
def _check_byr(pos, pp):
    if len(pos) == 4:
        if 1920 <= int(pos) <= 2002: return True
    return False
def _check_iyr(pos, pp):
    if len(pos) == 4:
        if 2010 <= int(pos) <= 2020: return True
    return False
def _check_eyr(pos, pp):
    if len(pos) == 4:
        if 2020 <= int(pos) <= 2030: return True
    return False
def _check_hgt(pos, pp):
    if re.match("[\d]{3}cm", pos):
        if 150 <= int(pos[0:3]) <= 193:
            return True
    if re.match("[\d]{2}in", pos):
        if 59 <= int(pos[0:2]) <= 76:
            return True
    return False
def _check_hcl(pos, pp):
    if re.match("^#[a-f0-9]{6}$", pos):
        return True
    return False
def _check_ecl(pos, pp):
    if pos in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']:
        return True
    return False
def _check_pid(pos, pp):
    if len(pos) == 9: return True
    return False
def _check_cid(pos, pp):
    return True

if __name__ == '__main__':
    checks1 = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid']
    checks2 = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid', 'cid']
    # _input = [
    #     [
