...
def _pt1():
    ...
    for e in _input:
        _min = int(e[0].split('-')[0])
        _max = int(e[0].split('-')[1])
        _c = e[1][0]
        _pass = e[2]

        if _min <= _pass.count(_c) <= _max: valid += 1
...
def _pt2():
    ...
    for e in _input:
        _first = int(e[0].split('-')[0])
        _second = int(e[0].split('-')[1])
        _c = e[1][0]
        _pass = e[2]

        if len(_pass) >= _second:
            if (_pass[_first-1] == _c and _pass[_second-1] != _c) or (_pass[_first-1] != _c and _pass[_second-1] == _c): valid += 1
